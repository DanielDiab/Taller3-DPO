...
public class GeneradorTiquetes {
    ...
    public static Tiquete generarTiquete(Vuelo vuelo, Cliente cliente, int tarifa) {
        int numero = (int) (Math.random() * 1_000_0000); // Generar un número aleatorio de 7 dígitos
        String codigo = String.format("%07d", numero);
        
        while (codigos.contains(codigo)) {
            numero = (int) (Math.random() * 1_000_0000);
            codigo = String.format("%07d", numero);
        }

        Tiquete nuevoTiquete = new Tiquete(codigo, vuelo, cliente, tarifa);
        registrarTiquete(nuevoTiquete); // Registrar el tiquete generado
        return nuevoTiquete;
    }
